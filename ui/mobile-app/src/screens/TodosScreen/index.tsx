...
type ToDoScreenNavigationProp = StackNavigationProp<AuthStackParamList, 'ToDoScreen'>;
  ...
  function RenderHeader() {
    return (
      <View
        ...
        <Pressable onPress={callEditTitle} style={{ marginLeft: 5, justifyContent: 'center', marginTop: -8 }}>
          <Image source={editTitle ? icons.done : icons.edit} style={{ height: 25, width: 30, tintColor: 'white', resizeMode: 'contain' }} />
        </Pressable>
        <View style={{ flexDirection: 'row', marginLeft: 'auto', marginTop: -2 }}>
          {
            addedUsers?.slice(-5).map((user, index) => (
              <Pressable onPress={() => setShowUsers(true)} key={user.id} style={{ marginLeft: -20 }}>
        ...
        <View style={{ marginLeft: 10, marginTop: -2 }}>
          <AddButton onPress={toggleListModal} dimension={30} icon={showAddUser || showUsers ? 'cross' : 'plus'} />
        </View>
      </View>
    )
  }
